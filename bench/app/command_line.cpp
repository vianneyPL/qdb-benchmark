...
static const bench::test_class *get_test_by_id(
    const bench::test_collection &tests, const std::string &id) {
  for (auto test : tests) {
...
void bench::app::command_line::parse(int argc, const char **argv) {
  ...
  bool version =
      parser.get_flag("-v", "--version", "Display program version and exists");
  bool help =
      parser.get_flag("-h", "--help", "Display program help and exists");
  _settings.cluster_uri = parser.get_string(
      "-c", "--cluster", "Set cluster URI", "qdb://127.0.0.1:2836");
  _settings.pause = std::chrono::seconds(parser.get_integer(
      "-p", "--pause", "Set the delay between each test, in seconds", "1"));
  _settings.duration = std::chrono::seconds(parser.get_integer(
      "-d", "--duration", "Set the duration of each test, in seconds", "4"));
  _settings.thread_counts =
      parser.get_integers("", "--threads", "Set number of threads", "1,2,4,8");
  _settings.content_sizes =
      parser.get_values<std::size_t>("", "--sizes", "Set contents sizes",
  ...
  _settings.tests = parser.get_values<const test_class *>(
      ...
      [this](const std::string &id) { return get_test_by_id(_test_pool, id); });
