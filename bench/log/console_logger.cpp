...
{
    std::cout << "Wait " << std::chrono::duration_cast<std::chrono::seconds>(duration).count() << " seconds";
...
void bench::log::console_logger::test_started(int num, int total, const test_instance & test)
{
    ...
    if (test.tclass.size_dependent)
        std::cout << "  - content size = " << make_size_string(test.config.content_size) << std::endl;
}
...
void bench::log::console_logger::test_finished(int num, int total, const test_instance & test)
{
    if (test.tclass.size_dependent)
    {
        std::cout << "Average throughput = " << make_size_string((size_t)compute_average_throughput(test)) << "/s";
    }
    else
    {
