...
{
public:
    explicit col_double_average_part(bench::test_config config) : qdb_test_template(config), _ts_size(config.content_size)
    {
    }
    ...
    void run_iteration(std::uint32_t iteration)
    {
        ...
        static std::uniform_int_distribution<idb_time_t>   dist(_inserted_range.begin.tv_sec, _inserted_range.end.tv_sec - part);
