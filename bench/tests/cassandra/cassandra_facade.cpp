...
cassandra_facade::cassandra_facade() : _cluster(cass_cluster_new()), _session(cass_session_new())
{
}
...
void cassandra_facade::connect(const std::string & cluster_uri)
{
    ...
    if (rc != CASS_OK) throw std::runtime_error("Unable to connect: " + cluster_uri);
...
/* static */ std::vector<std::string> cassandra_facade::resolve_topology(const std::string & node_uri)
{
    ...
    cass_future_wait(connect_future);
    CassError rc = cass_future_error_code(connect_future);
    if (rc != CASS_OK) throw std::runtime_error("Unable to connect: " + node_uri);
...
void cassandra_facade::remove(const std::string & alias)
{
    ...
    cass_future_free(execute(_session, statement));
    cass_statement_free(statement);
}
...
std::string cassandra_facade::blob_get(const std::string & alias)
{
    ...
    CassIterator * iterator = cass_iterator_from_result(result);
    if (!cass_iterator_next(iterator)) throw std::runtime_error("object not found: " + alias);
...
/* static */ CassFuture * cassandra_facade::execute(CassSession * session, CassStatement * statement)
{
